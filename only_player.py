...
def runGame(FPS, clock):
	run = True
	timer = 0
	player_fireballs_on_screen = 4
	player = Player(WIDTH/2, HEIGHT/2, "red")
	player_fireballs = createFireballs(player.rect.center[0], player.rect.center[1], "blue")
	...
	while(run):
		...
		if(player_fireballs_on_screen == 0):
			player_fireballs = createFireballs(player.rect.center[0], player.rect.center[1], "blue")
		...
		moveFireballs(player_fireballs)

		if(checkCollision(player, player_fireballs)):
			break

		player_fireballs_on_screen = countFireballsOnScreen(player_fireballs)
